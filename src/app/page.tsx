...
import { splitLines, toUniqueSet, type NormalizeOptions } from "@/lib/parse";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <section className="flex flex-col">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-xs tracking-wide uppercase font-normal text-gray-900 dark:text-gray-100">Input A</h2>
              <div className="text-[12px] text-gray-600 dark:text-gray-300">
                {results.aTotal} lines • {results.aUnique} unique
              </div>
            </div>
            <textarea
              value={textA}
              ...
              className="w-full h-[50vh] resize-none font-mono whitespace-pre-wrap break-words p-3 rounded-none border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:outline-none focus:border-gray-400 dark:focus:border-gray-600"
            />
          </section>

          <section className="flex flex-col">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-xs tracking-wide uppercase font-normal text-gray-900 dark:text-gray-100">Input B</h2>
              <div className="text-[12px] text-gray-600 dark:text-gray-300">
                {results.bTotal} lines • {results.bUnique} unique
              </div>
            </div>
            <textarea
              value={textB}
...
function CopyChips({
  ...
  return (
    <div className={className ?? "mt-6 flex flex-wrap gap-3"}>
      ...
      >
        <span className="text-[12px]">Only in A: {results.onlyA.size}</span>
      </button>
      <button
        type="button"
        disabled={cooldown === "onlyB"}
        onClick={() => copy("onlyB", results.onlyB)}
        className={`${base} ${cooldown === "onlyB" ? active : normal}`}
